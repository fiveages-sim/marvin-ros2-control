...
namespace marvin_ros2_control
{
    ...
    protected:
        ...
        static constexpr uint8_t kRightSlaveId = 0x01;
        static constexpr uint8_t kMaxProtocolAngle = 90;
        static constexpr uint8_t kDefaultCommandSpeed = 100;
            ...
            const std::vector<double>& positions) override
        {
            ...
            if (positions.size() != JOINT_COUNT)
            {
                RCLCPP_ERROR(logger_, "freedom_v2: Invalid position size. Expected %zu, got %zu",
                             JOINT_COUNT, positions.size());
                return false;
            }
            if (channel_ == CAN_CHANNEL)
            {
                RCLCPP_ERROR(logger_, "freedom_v2: CAN channel is not supported; use Marvin 485 channel");
                return false;
            }
        ...
        void deinitialize() override
        {
            RCLCPP_INFO(logger_, "freedom_v2 hand deinitialized");
        }
